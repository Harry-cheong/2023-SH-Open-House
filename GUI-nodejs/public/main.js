  ...
  for (var i = 0; i < allBlocks.length; i++) {
    ...
    if (id === "w" || id === "for") {
      finalText =
        ...
        val +
        "_" +
        $(allBlocks[i]).attr("data-numblocks");
    } else {
...
function renderLocalStorage() {
  ...
  if (seq.length > 0) {
    for (var i = 0; i < seq.length; i++) {
      ...
      $(`#${window.codeCounter - 1}_${key} input`).val(
        parseInt(seq[i].split("_")[1])
      );
      if (key === "w" || key === "for") {
        $(`#${window.codeCounter - 1}_${key}`).attr(
          "data-numblocks",
          parseInt(seq[i].split("_")[2])
        );
      }
    }
  }
...
function addToCode(key) {
  contents = $(`#${key}`).html();
  copy = $(
    `<button id="${window.codeCounter}_${key}" class="block fillblock ${allBlocks[key].color}"></button>`
  );
  ...
  if (key === "w" || key === "for") {
    ...
    $(`#${window.codeCounter}_${key} button.addlayer`).on(
      "click",
      function (e) {
        addlayer(e);
      }
    );
    $(`#${window.codeCounter}_${key} button.removelayer`).on(
      "click",
      function (e) {
        removelayer(e);
      }
...
const allBlocks = {
  f: {
    pretext: "Move forward ",
    posttext: "°",
    inputRequired: true,
    inputType: "number",
    max: 10000,
    min: 1,
    info: "Moves the robot forward by degrees of wheel rotation",
    color: "orange",
  },
  r: {
    pretext: "Turn right ",
    ...
    max: 10000,
    min: 1,
    info: "Turns the robot right by degrees of robot",
    color: "orange",
  },
  l: {
    pretext: "Turn left ",
    ...
    max: 10000,
    min: 1,
    info: "Moves the robot left by degrees of robot",
    color: "orange",
  },
  b: {
    pretext: "Move backward ",
    posttext: "°",
    inputRequired: true,
    inputType: "number",
    max: 10000,
    min: 1,
    info: "Moves the robot backward by degrees of wheel rotation",
    color: "orange",
  },
  t: {
    pretext: "Line trace ",
    ...
    max: 10000,
    min: 1,
    info: "Moves the robot forward by tracing the line by degrees of wheel rotation",
    color: "orange",
  },
  w: {
    pretext: "Repeat continuously",
    posttext: "",
    inputRequired: false,
    info: "Repeats code blocks within it infinitely",
    color: "blue",
  },
  for: {
    pretext: "Repeat ",
    ...
    max: 10000,
    min: 1,
    info: "Repeats code blocks within it a set number of times",
    color: "blue",
  },
};
